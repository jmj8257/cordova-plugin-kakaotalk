...
- (void) share:(CDVInvokedUrlCommand *)command {
    ...
    NSString *paramsTitle = @"";
    NSString *paramsDesc = @"";
    NSString *paramsImageUrl = @"";
    NSString *paramsLink = @"";

    // 파라미터 파싱
    if (options[@"params"]) {
        NSDictionary *paramsObj = options[@"params"];
        paramsTitle = paramsObj[@"title"] ?: @"";
        paramsDesc = paramsObj[@"desc"] ?: @"";
        paramsImageUrl = paramsObj[@"imageUrl"] ?: @"";
        paramsLink = paramsObj[@"link"] ?: @"";
    }

    NSString *webLinkText = options[@"weblink"][@"text"] ?: @"";
    NSString *webLinkUrl = options[@"weblink"][@"url"] ?: @"";
    NSString *appLinkText = options[@"applink"][@"text"] ?: @"";
    NSString *appLinkUrl = options[@"applink"][@"url"] ?: @"";

    // 템플릿 생성
    KMTContentObject *content = [KMTContentObject contentObjectWithBuilderBlock:^(KMTContentBuilder * _Nonnull builder) {
        builder.title = paramsTitle;
        builder.desc = paramsDesc;
        builder.imageURL = [NSURL URLWithString:paramsImageUrl];
        builder.link = [KMTLinkObject linkObjectWithBuilderBlock:^(KMTLinkBuilder * _Nonnull linkBuilder) {
            linkBuilder.mobileWebURL = [NSURL URLWithString:paramsLink];
        }];
    }];

    NSArray<KMTButtonObject *> *buttons = @[
        [KMTButtonObject buttonObjectWithBuilderBlock:^(KMTButtonBuilder * _Nonnull builder) {
            builder.title = webLinkText;
            builder.link = [KMTLinkObject linkObjectWithBuilderBlock:^(KMTLinkBuilder * _Nonnull linkBuilder) {
                linkBuilder.mobileWebURL = [NSURL URLWithString:webLinkUrl];
            }];
        }],
        [KMTButtonObject buttonObjectWithBuilderBlock:^(KMTButtonBuilder * _Nonnull builder) {
            builder.title = appLinkText;
            builder.link = [KMTLinkObject linkObjectWithBuilderBlock:^(KMTLinkBuilder * _Nonnull linkBuilder) {
                linkBuilder.mobileWebURL = [NSURL URLWithString:appLinkUrl];
            }];
        }]
    ];

    KMTTemplate *template = [KMTFeedTemplate feedTemplateWithBuilderBlock:^(KMTFeedTemplateBuilder * _Nonnull builder) {
